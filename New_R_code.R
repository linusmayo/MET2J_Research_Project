...
filtered_instruments <- full_data |>
  ...
  #Counting instruments per every five years to plot e.g. 'total guitarists in 1965' 
...
filtered_genres <- genre_data |>
  pivot_longer(cols=-'Primary Genre', names_to="Year", values_to="Percentage per instrument") |>
...
plot_genres <- ggplot(data = filtered_genres, aes(x = Quinquennial, y = Percentage, color = `Primary Genre`)) + 
  geom_line(linewidth = 1) +
  labs(x= "Years", y="Percentage of songs in Hot 100", title="Top genres between 1960 and 1995") +
  xlim(c(1960, 1995)) +
  theme_light() +
  theme(plot.title = element_text(face="bold"))+
  scale_color_manual(values = c(
    "House/Electronic/Trance" = "black",
    "Jazz" = "gold",
    "Pop" = "lightgreen",
    "Rock" = "lightblue",
    "Folk" = "magenta3"
...
banjo_data <- filtered_instruments |>
  ...
  ggplot() +
  geom_line(data = guitar_data, aes(x = Quinquennial, y = Percentage), color = "darkgreen") + 
  geom_line(data = rock_data, aes(x = Quinquennial, y = Percentage), color = "lightblue") +
  sec.axis = sec_axis(name="Guitar prevalence") +
  labs(x= "Years", y="Rock prevalence", title="Relationship between Rock and Guitar") +
  xlim(c(1960, 1995)) +
  geom_segment(aes(x = 1990, y = 0, xend = 1990, yend = 30),linetype="dashed", color = "red") +
  geom_segment(aes(x = 1985, y = 0, xend = 1990, yend = 60),linetype="dashed", color = "red") 
